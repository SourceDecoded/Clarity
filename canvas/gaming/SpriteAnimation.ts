...
import CanvasCachePackage = require("./CanvasCache");
...
class SpriteAnimation extends Animation {
    ...
    constructor(imageCache: ImageCache, rigidBody: RigidBody, sprites: Sprite[] = []) {
        super();
    ...
    draw(context: CanvasRenderingContext2D, viewRect) {
        ...
        if (sprite && intersection && this.rigidBody) {
            ...
            context.drawImage(
                canvas,
                Math.ceil(spriteRect.x), Math.floor(spriteRect.y),
                Math.ceil(spriteRect.width),
                Math.ceil(spriteRect.height),
                Math.ceil(placementX + sprite.offset.x),
                Math.ceil(placementY + sprite.offset.y),
                Math.ceil(spriteRect.width),
                Math.ceil(spriteRect.height)
                );
